...
export function HeaderOptimized({ onMenuToggle }: HeaderProps) {
  ...
  // Memoize unread count to avoid recalculation
  const unreadCount = useMemo(
    () => notifications.filter(n => !n.read).length,
    [notifications]
  );
  
  // Memoize home href based on role
  const homeHref = useMemo(
    () => role === 'admin' ? ROUTES.COURSES : ROUTES.DASHBOARD,
    [role]
  );
  ...
  useEffect(() => {
    ...
    const loadNotifications = async () => {
      // In production, this should be a real API call with caching
      // For now, using mock data but only load once
      if (!mounted) return;
      
      // Mock notifications - in production use React Query
      const mockNotifications: Notification[] = Array.from({ length: 5 }, (_, i) => ({
        id: `notif-${i}`,
        title: "Cộng đồng",
        message: `Bạn được cộng ${i % 2 === 0 ? 500 : 3000} thóc`,
        read: false,
  ...
  useEffect(() => {
    const handleProfileUpdate = () => {
      // Use the global refreshProfile from auth context
      refreshProfile();
    };
    ...
    <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-4 md:px-6 sticky top-0 z-50">
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          ...
          className="p-2 hover:bg-gray-100 rounded-full relative notification-menu-container"
      ...
      {/* Notification Dropdown - Lazy rendered */}
      {notificationOpen && (
        <div className="absolute right-4 top-16 mt-2 w-80 bg-teal-50 rounded-lg shadow-lg border border-teal-200 py-2 z-50 max-h-96 flex flex-col notification-menu-container">
